...
df.null.euc.m <- sim.euc.dist.m(df.merge, pop.muts, sizes, df.m.groups, 10000)
# standardize mean centroid distances
...
# get 95% CIs from permutation results
# 1,000 * 0.025 = 25
LCI.m <- c(sort(df.null.euc.m.z.df$L0B)[250], sort(df.null.euc.m.z.df$L0S)[250],
        sort(df.null.euc.m.z.df$L1B)[250], sort(df.null.euc.m.z.df$L1S)[250],
        sort(df.null.euc.m.z.df$L2B)[250], sort(df.null.euc.m.z.df$L2S)[250])
df.null.euc.m.z.u.mat["LCI"] <- LCI.m
UCI.m <- c(sort(df.null.euc.m.z.df$L0B)[10000-250], sort(df.null.euc.m.z.df$L0S)[10000-250],
        sort(df.null.euc.m.z.df$L1B)[10000-250], sort(df.null.euc.m.z.df$L1S)[10000-250],
        sort(df.null.euc.m.z.df$L2B)[10000-250], sort(df.null.euc.m.z.df$L2S)[10000-250])
...
```{r}
png(filename = paste(c("figs/beta-disper-m.png"), collapse = ''),
    width = 1200, height = 1200, res = 96*2)
...
# Show Plot
img <- readPNG(paste(c("figs/beta-disper-m.png"), collapse = ''))
grid.raster(img)
```
